...
st.title("Crash Game Predictor (Adaptive Models)")
...
if st.sidebar.button("Reset history"):
    st.session_state.history = []
    st.sidebar.warning("History reset.")
...
def moving_average_model(data, window=5, threshold=2.0):
    ...
    avg = np.mean(recent)
    if avg > threshold:
        return 0.7, 0.3  # Strong bias to Above
    elif avg < threshold:
        return 0.3, 0.7
    else:
        return 0.5, 0.5
...
# Footer
st.markdown("---")
st.write("*Models: Frequency (global ratio), Moving Average (recent trend), Markov Chain (state transitions).*")
